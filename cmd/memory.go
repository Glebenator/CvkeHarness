...
var memoryReindexCmd = &cobra.Command{
	Use:        "reindex",
	Short:      "Deprecated alias for an explicit validated import from configured Markdown views",
	Deprecated: "use `cvkeharness memory import` instead",
	RunE: func(cmd *cobra.Command, args []string) error {
		mem, store, err := openMemoryManager()
		if err != nil {
			return err
		...
		defer store.Close()
		if err := mem.Import(context.Background(), ""); err != nil {
			return err
		}
		fmt.Println("Imported validated operational memory; SQLite remains canonical")
		return nil
	},
}
...
var memoryRollbackCmd = &cobra.Command{
	...
	Short: "Restore one generated view snapshot through the validated import boundary",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Printf("Restored snapshot %s through validated import\n", args[0])
		return nil
	},
}
