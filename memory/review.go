...
func (m *Manager) transitionMemory(ctx context.Context, kind, id, next string) error {
	...
	prepare := func(current, targetID, environment string, expiresAt *time.Time, trust *string) error {
		...
		targetEnv, targetLive := liveTargetEnvironment(st, targetID, now)
		if !targetLive || environment == "" || environment == state.EnvironmentUnknown || environment != targetEnv {
			return fmt.Errorf("cannot promote memory for a stale, provisional, unknown, or mismatched target environment")
		}
		...
		for targetIdx := range st.Targets {
			for factIdx := range st.Targets[targetIdx].Facts {
				...
				if item.EvidenceHash != factIntegrity(*item) {
					return fmt.Errorf("cannot transition fact %q because its evidence hash is invalid", id)
				}
	...
	case "playbook":
		for idx := range st.Playbooks {
			...
			if item.EvidenceHash != playbookIntegrity(*item) {
				return fmt.Errorf("cannot transition playbook %q because its evidence hash is invalid", id)
			}
			if next == state.MemoryStatusActive && (len(item.SuccessChecks) == 0 || item.LastVerifiedAt.IsZero() || strings.TrimSpace(item.EvidenceRef) == "") {
				return fmt.Errorf("cannot promote playbook %q without a verified outcome, verification evidence, and an explicit success check", id)
			}
	...
	case "finding":
		for idx := range st.Findings {
			...
			if item.EvidenceHash != findingIntegrity(*item) {
				return fmt.Errorf("cannot transition finding %q because its evidence hash is invalid", id)
			}
			if err := prepare(item.Status, item.TargetID, item.Environment, &item.ExpiresAt, &item.Trust); err != nil {
				return err
			}
	...
	case "caution":
		for idx := range st.Cautions {
			...
			if item.EvidenceHash != cautionIntegrity(*item) {
				return fmt.Errorf("cannot transition caution %q because its evidence hash is invalid", id)
			}
			if err := prepare(item.Status, item.TargetID, item.Environment, &item.ExpiresAt, &item.Trust); err != nil {
				return err
			}
