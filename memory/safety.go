...
func expireOperationalItems(st *fileState, now time.Time) bool {
	if st == nil {
		return false
	}
	changed := false
	expire := func(status *string, expiresAt time.Time) {
		if (*status == state.MemoryStatusCandidate || *status == state.MemoryStatusActive) &&
			!expiresAt.IsZero() && !expiresAt.After(now) {
			*status = state.MemoryStatusExpired
			changed = true
		}
	}
	for targetIdx := range st.Targets {
		expire(&st.Targets[targetIdx].Target.Status, st.Targets[targetIdx].Target.ExpiresAt)
		for factIdx := range st.Targets[targetIdx].Facts {
			expire(&st.Targets[targetIdx].Facts[factIdx].Status, st.Targets[targetIdx].Facts[factIdx].ExpiresAt)
		}
	}
	for idx := range st.Playbooks {
		expire(&st.Playbooks[idx].Status, st.Playbooks[idx].ExpiresAt)
	}
	for idx := range st.Findings {
		expire(&st.Findings[idx].Status, st.Findings[idx].ExpiresAt)
	}
	for idx := range st.Cautions {
		expire(&st.Cautions[idx].Status, st.Cautions[idx].ExpiresAt)
	}
	return changed
}
