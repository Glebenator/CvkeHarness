...
func TestEnsureFilesRepairsGeneratedViewDriftFromCanonicalSQLite(t *testing.T) {
	t.Parallel()
	...
	manager.hostname = func() string { return "runtime.local" }
	ctx := context.Background()
	if err := manager.PersistLessons(ctx, []Lesson{{Body: "Inspect the service status before mutation.", Confidence: 0.8}}); err != nil {
		t.Fatalf("PersistLessons returned error: %v", err)
	}
	...
	if strings.Contains(string(data), "INJECTED VIEW CONTENT") || !strings.Contains(string(data), "Inspect the service status") {
		t.Fatalf("generated view was not repaired from canonical SQLite: %s", data)
	}
	snapshots, err := store.ListSnapshots(ctx)
	if err != nil {
		t.Fatalf("ListSnapshots returned error: %v", err)
	...
	found := false
	for _, snapshot := range snapshots {
		if snapshot.SourceFile == FindingsFile && snapshot.Reason == "refresh generated views from canonical SQLite" {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected drift repair snapshot, got %#v", snapshots)
	}
}
...
func TestLegacyMarkdownMigrationSnapshotsAndQuarantinesOperationalKnowledge(t *testing.T) {
	t.Parallel()
...
kind: ssh
primary_name: old-prod.internal
transport: ssh
status: active
...
` + "```\n\n" + `Use the old package mirror during maintenance.
`
	for name, content := range map[string]string{
		TargetsFile: legacyTargets, FindingsFile: legacyFindings,
		PlaybooksFile: "# Playbooks\n\n", CautionsFile: "# Cautions\n\n",
	} {
	...
	st, err := manager.loadState(context.Background())
	if err != nil {
		t.Fatalf("loadState returned error: %v", err)
	}
	if len(st.Findings) != 1 {
		t.Fatalf("expected one migrated finding, got %#v", st.Findings)
	}
	finding := st.Findings[0]
	if finding.Status != state.MemoryStatusCandidate || finding.Trust != state.MemoryTrustUntrusted || finding.Source != "legacy_markdown_migration" {
		t.Fatalf("legacy finding was not quarantined: %#v", finding)
	}
	if finding.Environment != state.EnvironmentUnknown || finding.EvidenceHash != findingIntegrity(finding) {
		t.Fatalf("legacy finding scope/integrity mismatch: %#v", finding)
	}
	targetIdx := targetIndex(st, "target-old")
	if targetIdx < 0 || st.Targets[targetIdx].Target.Status != state.MemoryStatusCandidate || st.Targets[targetIdx].Target.RemoteIdentity != "" {
		t.Fatalf("legacy remote target was not left provisional: %#v", st.Targets)
	}
	if _, live := liveTargetEnvironment(st, "target-old", time.Now().UTC()); live {
		t.Fatal("legacy remote target must not become live during migration")
	}

	snapshots, err := store.ListSnapshots(context.Background())
	if err != nil || len(snapshots) == 0 {
		t.Fatalf("expected pre-migration snapshots, got %#v err=%v", snapshots, err)
	}
	for _, snapshot := range snapshots {
		if snapshot.Reason != "quarantine legacy Markdown as review candidates" {
			t.Fatalf("unexpected migration snapshot reason: %#v", snapshot)
		}
	}

	retrieved, err := manager.Retrieve(context.Background(), core.RetrievalContext{
		Task: "use the package mirror on old-prod.internal", TargetID: "target-old", TargetKind: TargetKindSSH,
	})
	if err != nil {
		t.Fatalf("Retrieve returned error: %v", err)
	}
	if retrieved.FallbackBrief != "" || strings.Contains(retrieved.TargetSummary, "package_manager") {
		t.Fatalf("quarantined legacy memory leaked into retrieval: %#v", retrieved)
	}
}

func TestUnverifiedPlaybookWithPostconditionCannotBePromoted(t *testing.T) {
	t.Parallel()
	...
	target, err := manager.ResolveTarget(ctx, TargetResolutionInput{Command: "ssh ops@prod-web systemctl status nginx"})
	if err != nil {
		t.Fatalf("ResolveTarget returned error: %v", err)
	}
	if err := manager.SetTargetEnvironment(ctx, target.TargetID, "production", "ops@prod-web"); err != nil {
	...
	target.Environment = "production"
	if err := manager.CurateRunOutcome(ctx, RunOutcome{
		Task: "restart nginx on prod-web", Target: target, VerifiedOutcome: false,
		ToolCalls: []ObservedToolCall{
			{ToolName: "shell_execute", TargetID: target.TargetID, Command: "ssh ops@prod-web systemctl restart nginx", Success: true},
			{ToolName: "shell_execute", TargetID: target.TargetID, Command: "ssh ops@prod-web systemctl is-active nginx", Result: "active", Success: true},
		},
	}); err != nil {
		t.Fatalf("CurateRunOutcome returned error: %v", err)
	}
	st, _ := manager.loadState(ctx)
	if len(st.Playbooks) != 1 || len(st.Playbooks[0].SuccessChecks) == 0 {
		t.Fatalf("expected candidate with inferred postcondition, got %#v", st.Playbooks)
	}
	if !st.Playbooks[0].LastVerifiedAt.IsZero() {
		t.Fatalf("exit success must not become verified outcome: %#v", st.Playbooks[0])
	}
	if err := manager.PromoteMemory(ctx, "playbook", st.Playbooks[0].ID); err == nil || !strings.Contains(err.Error(), "verified outcome") {
		t.Fatalf("expected unverified playbook promotion to fail, got %v", err)
	}
}

func TestPromotionRejectsTamperedCandidateEvidence(t *testing.T) {
	t.Parallel()
	...
	manager.hostname = func() string { return "runtime.local" }
	ctx := context.Background()
	if err := manager.PersistLessons(ctx, []Lesson{{Body: "Check the service health endpoint before restart.", Confidence: 0.8}}); err != nil {
		t.Fatalf("PersistLessons returned error: %v", err)
	}
	st, _ := manager.loadState(ctx)
	st.Findings[0].Body = "tampered operational instruction"
	if err := store.ReplaceOperationalMemory(ctx, operationalMemoryFromState(st)); err != nil {
		t.Fatalf("ReplaceOperationalMemory returned error: %v", err)
	}
	if err := manager.PromoteMemory(ctx, "finding", st.Findings[0].ID); err == nil || !strings.Contains(err.Error(), "evidence hash is invalid") {
		t.Fatalf("expected tampered promotion to fail closed, got %v", err)
	}
}

func TestExpiredLifecycleIsPersistedAndWithheld(t *testing.T) {
	t.Parallel()
	...
	ctx := context.Background()
	if err := manager.PersistLessons(ctx, []Lesson{{Body: "Inspect the bounded health log before repair.", Confidence: 0.8}}); err != nil {
		t.Fatalf("PersistLessons returned error: %v", err)
	}
	st, _ := manager.loadState(ctx)
	id := st.Findings[0].ID
	...
	manager.now = func() time.Time { return now.Add(activeTTL + time.Second) }
	if err := manager.EnsureFiles(); err != nil {
		t.Fatalf("EnsureFiles after expiry returned error: %v", err)
	}
	st, _ = manager.loadState(ctx)
	if st.Findings[0].Status != state.MemoryStatusExpired {
		t.Fatalf("expected persisted expired status, got %#v", st.Findings[0])
	}
	if err := manager.PromoteMemory(ctx, "finding", id); err == nil {
		t.Fatal("expired memory must not be reactivated")
	}
}
