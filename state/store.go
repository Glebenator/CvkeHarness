...
func migrate(ctx context.Context, db *sql.DB) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS scoped_command_approvals (
			...
			PRIMARY KEY(target_id, environment, command, action, policy_version)
		);`,
	...
	optionalColumns := []string{
		...
		`ALTER TABLE scheduled_jobs ADD COLUMN blocked INTEGER NOT NULL DEFAULT 0`,
		...
		`ALTER TABLE scoped_command_approvals ADD COLUMN session_id TEXT NOT NULL DEFAULT ''`,
